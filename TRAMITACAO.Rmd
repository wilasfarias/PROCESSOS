...
library(leaflet)
#library(pacman)
pacman::p_load(rmarkdown,ggplot2,htmlTable,htmltools,markdown,dplyr,tidyr,readr,readxl,DT,knitr,shiny,dashboardthemes,purrr,fontawesome,rbokeh,bslib,plotly,htmlwidgets,digest,bit,bubblyr,dygraphs,mgcv,leaflet)
...
# colocando zero (0) no lugar nos missings no tempo de sobrestamento:
tramitacao_set$`TEMPO DE SOBRESTAMENTO (DIAS)`[is.na(tramitacao_set$`TEMPO DE SOBRESTAMENTO (DIAS)`)] <- 0

# colocar a data de hoje nos dados missing
tramitacao_set$`DATA DE ENCAMINHAMENTO (SAÍDA)`[is.na(tramitacao_set$`DATA DE ENCAMINHAMENTO (SAÍDA)`)] <- Sys.Date()
...
# ordenar dados
tramitacao_set <- tramitacao_set %>% arrange(tramitacao_set[,2], tramitacao_set[,1])
...
tramitacao_out <- read.csv('https://raw.githubusercontent.com/wilasfarias/PROCESSOS/main/Processo_2022/PROPLAN_OUTUBRO.csv', sep = ";", fileEncoding = "Latin1", header = TRUE, skip = 1)
...
# colocando zero (0) no lugar nos missings no tempo de sobrestamento:
tramitacao_out$`TEMPO DE SOBRESTAMENTO (DIAS)`[is.na(tramitacao_out$`TEMPO DE SOBRESTAMENTO (DIAS)`)] <- 0

# colocar a dta de hoje nos dados missing
tramitacao_out$`DATA DE ENCAMINHAMENTO (SAÍDA)`[is.na(tramitacao_out$`DATA DE ENCAMINHAMENTO (SAÍDA)`)] <- Sys.Date()
...
# ordenar dados
tramitacao_out <- tramitacao_out %>% arrange(tramitacao_out[,2], tramitacao_out[,1])
...
tramitacao_nov <- read.csv('https://raw.githubusercontent.com/wilasfarias/PROCESSOS/main/Processo_2022/PROPLAN_NOVEMBRO.csv', sep = ";", fileEncoding = "Latin1", header = TRUE, skip = 2)
...
# colocando zero (0) no lugar nos missings no tempo de sobrestamento:
tramitacao_nov$`TEMPO DE SOBRESTAMENTO (DIAS)`[is.na(tramitacao_nov$`TEMPO DE SOBRESTAMENTO (DIAS)`)] <- 0

# colocar a dta de hoje nos dados missing
tramitacao_nov$`DATA DE ENCAMINHAMENTO (SAÍDA)`[is.na(tramitacao_nov$`DATA DE ENCAMINHAMENTO (SAÍDA)`)] <- Sys.Date()
...
# ordenar dados
tramitacao_nov <- tramitacao_nov %>% arrange(tramitacao_nov[,2], tramitacao_nov[,1])
...
# colocando zero (0) no lugar nos missings no tempo de sobrestamento:
tramitacao_dez$`TEMPO DE SOBRESTAMENTO (DIAS)`[is.na(tramitacao_dez$`TEMPO DE SOBRESTAMENTO (DIAS)`)] <- 0

# colocar a dta de hoje nos dados missing
tramitacao_dez$`DATA DE ENCAMINHAMENTO (SAÍDA)`[is.na(tramitacao_dez$`DATA DE ENCAMINHAMENTO (SAÍDA)`)] <- Sys.Date()
...
# ordenar dados
tramitacao_dez <- tramitacao_dez %>% arrange(tramitacao_dez[,2], tramitacao_dez[,1])
...
sliderInput("dias", "Dias:", value = 100, min = 0, max = 100)
```
